...
int main(int argc, char** argv)
{
  ...
  for(int i = 1; i <= 7; ++i)
  {
    for(int j = 1; j <= 4; ++j)
    {
      std::string strNum = std::to_string(j * 10 + i);
      helloVk.loadModel(nvh::findFile("media/scenes/gumtooth/" + strNum + ".obj", defaultSearchPaths, true));
    }
  }
  //helloVk.loadModel(nvh::findFile("media/scenes/gumtooth/GumLow.obj", defaultSearchPaths, true));
  //helloVk.loadModel(nvh::findFile("media/scenes/gumtooth/GumUp.obj", defaultSearchPaths, true));
